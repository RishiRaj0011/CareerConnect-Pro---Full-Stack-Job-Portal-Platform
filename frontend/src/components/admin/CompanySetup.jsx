...
const CompanySetup = () => {
    const { id } = useParams();
    const { data: singleCompany = {} } = useGetCompanyById(id);
    ...
    const submitHandler = async (e) => {
        ...
        try {
            ...
            if (res.data.success) {
                // Invalidate cache so Companies list and this company refetch fresh data
            ...
            <div className='max-w-xl mx-auto my-10'>
                <form onSubmit={submitHandler}>
                    ...
                    <div className='grid grid-cols-2 gap-4'>
                        <div><Label>Company Name</Label><Input type="text" name="name" value={input.name} onChange={changeEventHandler} /></div>
                        <div><Label>Description</Label><Input type="text" name="description" value={input.description} onChange={changeEventHandler} /></div>
                        <div><Label>Website</Label><Input type="text" name="website" value={input.website} onChange={changeEventHandler} /></div>
                        <div><Label>Location</Label><Input type="text" name="location" value={input.location} onChange={changeEventHandler} /></div>
                        <div><Label>Logo</Label><Input type="file" accept="image/*" onChange={changeFileHandler} /></div>
                    </div>
                    {loading
